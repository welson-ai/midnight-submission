...
import { PrivacyLoanCreditScorer, witnesses, type PrivacyLoanCreditScorerPrivateState } from 'privacyloan-credit-scorer-contract';

// Type for the PrivacyLoan contract
type PrivacyLoanCreditScorerContract = PrivacyLoanCreditScorer.Contract<PrivacyLoanCreditScorerPrivateState>;
import { saveLoanProfile } from '../utils/loanProfiles';

export type PrivacyLoanCircuitKeys = 'requestLoan' | 'changePin' | 'blacklistUser' | 'removeBlacklistUser' | 'rotateAdmin' | 'respondToLoan' | 'registerProvider' | 'removeProvider';
...
export interface DeployedPrivacyLoan {
...
export type PrivacyLoanDeployment = IdleDeployment | InProgressDeployment | DeployedPrivacyLoan | FailedDeployment;

export interface PrivacyLoanAPIProvider {
  readonly deployment$: Observable<PrivacyLoanDeployment>;
  readonly privateState: PrivacyLoanCreditScorerPrivateState;
  readonly setPrivateState: (state: PrivacyLoanCreditScorerPrivateState) => void;
...
export const PrivacyLoanContext = createContext<PrivacyLoanAPIProvider | undefined>(undefined);

export type PrivacyLoanProviderProps = PropsWithChildren<{
  logger: Logger;
}>;

export const PrivacyLoanProvider: React.FC<PrivacyLoanProviderProps> = ({ logger, children }) => {
...
export const PrivacyLoanProvider: React.FC<Readonly<PrivacyLoanProviderProps>> = ({ logger, children }) => {
  ...
  const [privateState, setPrivateState] = useState<PrivacyLoanCreditScorerPrivateState>(() => {
  ...
  const [deploymentSubject] = useState(() => new BehaviorSubject<PrivacyLoanDeployment>({ status: 'idle' }));
  ...
  const privateStateProvider = useMemo(
    () => inMemoryPrivateStateProvider<string, PrivacyLoanCreditScorerPrivateState>(),
    []
  );
  ...
  const initializeProviders = useCallback(async () => {
    ...
    const zkConfigProvider: ZKConfigProvider<PrivacyLoanCircuitKeys> = new FetchZkConfigProvider<PrivacyLoanCircuitKeys>(zkConfigPath, fetch.bind(window));
  ...
  const deployNewContract = useCallback(async () => {
    try {
      ...
      setFlowMessage('Deploying PrivacyLoan Credit Scorer contract...');
      logger.info('Deploying PrivacyLoan Credit Scorer contract...');

      // Create compiled contract using the stable API pattern
      const zkConfigPath = window.location.origin;
      const privacyLoanCompiledContract = CompiledContract.make<PrivacyLoanCreditScorerContract>(
        'PrivacyLoanCreditScorer',
        PrivacyLoanCreditScorer.Contract
      ).pipe(
  ...
  const apiProvider: PrivacyLoanAPIProvider = {
  ...
  return <PrivacyLoanContext.Provider value={apiProvider}>{children}</PrivacyLoanContext.Provider>;
};
