...
import { PrivacyLoanCreditScorer, type PrivacyLoanCreditScorerPrivateState, witnesses } from '@contract/privacyloan-credit-scorer/contract/index.js';
...
import {
  type PrivacyLoanCreditScorerContract,
  type PrivacyLoanCreditScorerPrivateStateId,
  type PrivacyLoanCreditScorerProviders,
  type DeployedPrivacyLoanCreditScorerContract,
  type PrivacyLoanCreditScorerCircuits,
} from './common-types';
...
export const getZKLoanLedgerState = async (
  providers: PrivacyLoanCreditScorerProviders,
  contractAddress: ContractAddress,
): Promise<PrivacyLoanCreditScorer.Ledger | null> => {
  assertIsContractAddress(contractAddress);
  logger.info('Checking contract ledger state...');
  const state = await providers.publicDataProvider
    .queryContractState(contractAddress)
    .then((contractState) => (contractState != null ? PrivacyLoanCreditScorer.ledger(contractState.data) : null));
  return state;
};

// Create compiled contract using the stable API pattern
export const zkLoanCompiledContract = CompiledContract.make<PrivacyLoanCreditScorerContract>(
...
): Promise<DeployedPrivacyLoanCreditScorerContract> => {
...
export const deploy = async (
  providers: PrivacyLoanCreditScorerProviders,
  privateState: PrivacyLoanCreditScorerPrivateState,
): Promise<DeployedPrivacyLoanCreditScorerContract> => {
...
const { pureCircuits } = PrivacyLoanCreditScorer;
...
export const requestLoan = async (
  contract: DeployedPrivacyLoanCreditScorerContract,
  providers: PrivacyLoanCreditScorerProviders,
  ...
  const updatedState: PrivacyLoanCreditScorerPrivateState = {
    ...currentState,
...
export const displayContractState = async (
  providers: PrivacyLoanCreditScorerProviders,
  contract: DeployedPrivacyLoanCreditScorerContract,
): Promise<{ ledgerState: PrivacyLoanCreditScorer.Ledger | null; contractAddress: string }> => {
...
export const configureProviders = async (
  walletContext: WalletContext,
  config: Config,
): Promise<PrivacyLoanCreditScorerProviders> => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<PrivacyLoanCreditScorerCircuits>(contractConfig.zkConfigPath);

  return {
    privateStateProvider: levelPrivateStateProvider<typeof PrivacyLoanCreditScorerPrivateStateId>({
